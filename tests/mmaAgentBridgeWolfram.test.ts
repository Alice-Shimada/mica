...
it("supports abort evaluation requests without claiming confirmed abort", () => {
    ...
    expect(source).not.toContain('NotebookEvaluate[notebook, "Quit[]", InsertResults -> False]');
  ...
  it("restarts only a distinct notebook kernel and verifies its identity changed", () => {
    const killStart = source.indexOf("KillKernelRequest[args_Association]");
    ...
    const runStart = source.indexOf("RunCellRequest[args_Association]");
    const abortStart = source.indexOf("AbortEvaluationRequest[args_Association]", runStart);
    const killBody = source.slice(killStart, restartStart);
    const restartBody = source.slice(restartStart, restartEnd);
    const runBody = source.slice(runStart, abortStart);
    const killAbortIndex = killBody.indexOf('FrontEndTokenExecute[notebook, "EvaluatorAbort"]');
    const killIdentityIndex = killBody.indexOf("targetIdentity = If[wasRunning && StringQ[cachedIdentity], cachedIdentity, NotebookKernelIdentity[notebook]]");
    const killQuitIndex = killBody.indexOf("QuitNotebookKernel[notebook]");
    const killFinishAbortedIndex = killBody.indexOf('FinishRunningCell["aborted"]');
    const restartAbortIndex = restartBody.indexOf('FrontEndTokenExecute[notebook, "EvaluatorAbort"]');
    const targetIdentityIndex = restartBody.indexOf("targetIdentity = If[wasRunning && StringQ[cachedIdentity], cachedIdentity, NotebookKernelIdentity[notebook]]");
    ...
    const quitIndex = restartBody.indexOf("QuitNotebookKernel[notebook]");
    const changedIdentityCheckIndex = restartBody.indexOf("restartedIdentity === targetIdentity");
    const restartFinishAbortedIndex = restartBody.indexOf('FinishRunningCell["aborted"]');
    ...
    expect(source).toContain("RunTemporaryNotebookCell[notebook_NotebookObject, code_String, tag_String]");
    expect(source).toContain('NotebookWrite[notebook, Cell[BoxData[code], "Input", CellOpen -> False, CellTags -> {tag}], None]');
    expect(source).toContain('FrontEndTokenExecute[notebook, "EvaluateCells"]');
    expect(source).toContain("QuitNotebookKernel[notebook_NotebookObject]");
    expect(source).toContain("NotebookKernelProcessRunningQ[targetIdentity_String]");
    expect(source).toContain("ForceKillNotebookKernelProcess[targetIdentity_String]");
    expect(source).toContain("ProcessObject[processId]");
    expect(source).toContain('StringStartsQ[ToLowerCase[FileNameTake[program]], "wolframkernel"]');
    expect(source).toContain("KillProcess[process]");
    expect(source).not.toContain('NotebookEvaluate[notebook, "ToString[$SessionID]');
    expect(runBody).toContain("kernelIdentity = NotebookKernelIdentity[notebook]");
    expect(runBody).toContain('"kernelIdentity" -> kernelIdentity');
    expect(killBody).toContain('cachedIdentity = Lookup[record, "kernelIdentity", Missing["NotAvailable"]]');
    expect(killBody).toContain("targetIdentity = If[wasRunning && StringQ[cachedIdentity], cachedIdentity, NotebookKernelIdentity[notebook]]");
    expect(killBody).toContain("If[MatchQ[quitResult, _Failure], quitResult = ForceKillNotebookKernelProcess[targetIdentity]]");
    expect(killBody).toContain("If[TrueQ[quitResult] && NotebookKernelProcessRunningQ[targetIdentity], quitResult = ForceKillNotebookKernelProcess[targetIdentity]]");
    expect(restartBody).toContain('cachedIdentity = Lookup[record, "kernelIdentity", Missing["NotAvailable"]]');
    expect(restartBody).toContain("targetIdentity = If[wasRunning && StringQ[cachedIdentity], cachedIdentity, NotebookKernelIdentity[notebook]]");
    expect(restartBody).toContain("If[MatchQ[quitResult, _Failure], quitResult = ForceKillNotebookKernelProcess[targetIdentity]]");
    expect(restartBody).toContain("If[TrueQ[quitResult] && NotebookKernelProcessRunningQ[targetIdentity], quitResult = ForceKillNotebookKernelProcess[targetIdentity]]");
    expect(killAbortIndex).toBeGreaterThanOrEqual(0);
    expect(killIdentityIndex).toBeGreaterThan(killAbortIndex);
    expect(killQuitIndex).toBeGreaterThan(killIdentityIndex);
    expect(killFinishAbortedIndex).toBeGreaterThan(killQuitIndex);
    expect(restartAbortIndex).toBeGreaterThanOrEqual(0);
    expect(targetIdentityIndex).toBeGreaterThan(restartAbortIndex);
    ...
    expect(changedIdentityCheckIndex).toBeGreaterThan(quitIndex);
    expect(restartFinishAbortedIndex).toBeGreaterThan(changedIdentityCheckIndex);
  ...
  it("reports abort_requested until abort completion is observed", () => {
    ...
    const abortedIndex = scanBody.indexOf('sameRunningCellQ && NumberQ[$AbortRequestedAt] && (evaluationCompleteQ || hasFinalOutputQ)');
    const requestedIndex = scanBody.indexOf('sameRunningCellQ && NumberQ[$AbortRequestedAt],');
    ...
    expect(requestedIndex).toBeGreaterThan(abortedIndex);
